...
def sort_out():
    ...
    while Sortout_Mode:
        ...
        if stopper_count == 2: 
            stopper_01.value = False
            print("sort_stopper_open")

        if Sortout_Mode:

            #print("current  before :",current_41)
            time.sleep(0.6)
            
            stopper_01.value = True
            print("sort_stopper_close")
            

            if current_41 > 2:
                #print("current   :",current_41)

                pucher_01.value = True
                time.sleep(1.4)
                pucher_01.value = False # red led off
                
                    # t1 = Thread(target=open_close_pusher, args=(2,), daemon=False)
                    # t1.start()
                    # t1.join()
                time.sleep(0.5)
                Sortout_Mode = False
...
global stopper_count z
    ...
    while True:
        ...
        while not do_skip and not itis_crawded:
            
###############################################################################################################################################

            current_40 = stopper_Sensor.current  # current in mA
            # current_41 = pusher_Sensor.current  # current in mA
            if (current_40 > 0.6) and (Once_Detect == False) and  Sortout_Mode == False:
                stopper_count = stopper_count + 1
                #print("Current_40  : {:7.4f}  mA".format(current_40))
                time.sleep(0.3)
                stopper_01.value = True
                print("detect_sort_stopper_close")
                time.sleep(0.2)
                
                do_detection=True
                time.sleep(0.5)
                stopper_01.value = False
                print("I did detection" , stopper_count)
            if current_40 > 0.6 and stopper_count == 2:
                print("I did detection2222222222" , stopper_count)
                do_detection=True
                stopper_01.value = False
                print("detect_sort22222_stopper_open")
                print("count_1", stopper_count)
                if stopper_count == 2:
                    stopper_01.value = True
                    print("detect_sort_stopper_close")
                    

                if not Sortout_Mode and stopper_count ==0:
                    time.sleep(0.5)
                    stopper_01.value = False
                    print("detect_sort_stopper_open")
                Once_Detect = True
                                
                #print("+++", Once_Detect , "+++",stopper_count,"+++", Sortout_Mode)
                
                break
            if current_40 < 0.6 :
                # print("count_0", stopper_count)
                #simulations the line
                time.sleep(0.2)
                do_skip=True
                # print("sensor_stopper_off ", do_detection)
                #print("Current_40  : {:7.4f}  mA".format(current_40))
                # print("Current_41  : {:7.4f}  mA".format(current_41))
                Once_Detect = False
