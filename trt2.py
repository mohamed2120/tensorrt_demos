...
def loop_and_detect(cam, trt_yolo, conf_th,save_labels,vis):
    ...
    while True:
        #check the window if it's created successfully
        ...
        if not is_app_ready:
            is_app_ready=True
            #create thread to activate leds in parallel to rest of the code pass the name function with the arguments
        ...
        #if user selected to skip frame just initialize do_skip to false and continue 
        if do_skip:
            do_skip=False
